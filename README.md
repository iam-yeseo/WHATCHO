...
- 신호를 한 번 받으면 브라우저가 응답 수신시각을 기준으로 잔여시간을 자체 계산합니다. 화면에는 소수점 이하를 버린 정수 초만 표시합니다. 가장 먼저 끝나는 표시 신호 단계 직후에 다시 동기화하고, 긴 단계는 60초에 한 번만 오차를 보정합니다. 실패 시에는 15초 뒤 재시도합니다.
- 사거리 약도는 내 진행 방향을 항상 위로 두고 직진·좌회전·우회전의 실제 방위와 3구 신호등을 함께 표시합니다. GPS 방향이 경계에서 조금 흔들려도 접근 방향이 바로 바뀌지 않도록 히스테리시스를 적용합니다.
